...
# ------------ CONFIG ------------
MODEL_PATH = "classifier2.pkl"
RECOGNITION_DURATION = 5.0        # seconds
SIM_THRESHOLD = 0.5               # cosine similarity threshold for known classes

# Liveness thresholds and duration
BLINK_CLOSED_THRESH = 0.22        # EAR threshold for closed eye
BLINK_OPEN_THRESH = 0.28          # EAR threshold for open eye
BLINK_CLOSED_FRAMES = 4           # consecutive frames to confirm closed
BLINK_OPEN_FRAMES = 4             # consecutive frames to confirm open
REQUIRED_BLINKS = 3               # required number of blinks
MIN_BLINK_DURATION = 0.1          # seconds
MAX_BLINK_DURATION = 0.8          # seconds

YAW_THRESH = 15.0                 # degrees (single detection)
...
def detect_blink(cam):
    ...
    while True:
        ...
        if lm is None:
            ...
            if state == 'open':
                if lm and ear < BLINK_CLOSED_THRESH:
                    closed_frames += 1
                ...
                if lm and ear > BLINK_OPEN_THRESH:
                    open_frames += 1
